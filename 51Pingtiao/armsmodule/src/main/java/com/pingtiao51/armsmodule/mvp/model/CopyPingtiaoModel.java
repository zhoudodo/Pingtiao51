 ...
 */
@FragmentScope
public class CopyPingtiaoModel extends BaseModel implements CopyPingtiaoContract.Model {
    @Inject
    Gson mGson;
    @Inject
    ...
    @Inject
    public CopyPingtiaoModel(IRepositoryManager repositoryManager) {
    ...
    public Observable<BaseJson<List<PingTiaoSeachResponse>>> getPingtiaoSearch() {
        return mRepositoryManager.obtainRetrofitService(HomeApi.class)
                .getPingTiaoSearch(new PingTiaoSeachRequest(AppUtils.getAppVersionName(),"PAPER","ANDRIOD"));
    }
}